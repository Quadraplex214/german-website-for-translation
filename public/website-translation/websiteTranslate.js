(function () {
  ...
  class DomUtils {
    ...
    static getScriptConfig() {
      ...
      let toggle = ((tag == null ? void 0 : tag.dataset.toggle) || "off")
        .toString()
        .trim()
        .toLowerCase();
      if (!["on", "off", "true", "false"].includes(toggle)) {
        toggle = "off";
      }
      toggle = toggle === "true" ? "on" : toggle === "false" ? "off" : toggle;
      return { apiKey, domain, toggle };
    }
    static isToggleOn(toggleValue) {
      if (typeof toggleValue === "string") {
        const v = toggleValue.trim().toLowerCase();
        return v === "on" || v === "true";
      }
  ...
  class LanguageDropdown {
    ...
    create() {
      if (document.getElementById(SELECTORS.DROPDOWN_CONTAINER)) {
    ...
    toggleDropdown() {
      ...
      const panel = this.container.querySelector(".tl-dropdown-panel");
      const btn = this.container.querySelector(".tl-translate-btn");
      if (!panel || !btn) return;
      const isOpen = panel.classList.toggle("open");
      btn.setAttribute("aria-expanded", String(isOpen));
      if (isOpen) {
        const input = panel.querySelector(".tl-search");
    ...
    generateCSS(theme) {
      ...
      return `
            ...
            /* Floating circular icon mode */
            #${SELECTORS.DROPDOWN_CONTAINER}.tl-floating-btn {
            position: fixed;
            bottom: var(--tl-bottom, 24px);
            right: var(--tl-right, 24px);
            z-index: var(--tl-z-index, 2147483647) !important;
            ...
            height: 48px;
            border-radius: 50%;
            background: var(--tl-translate-bg, #111);
            color: var(--tl-translate-color, #fff);
            border: none;
            display: inline-flex;
            align-items: center;
            justify-content: center;
            ...
            .tl-translate-btn:hover,
            .tl-translate-btn:focus {
            ...
            .tl-dropdown-panel {
            position: absolute;
            right: 60px; /* places panel to the right of the button */
            bottom: 0;
            display: none;
            min-width: 200px;
            max-width: 320px;
            ...
            color: var(--tl-panel-color, ${
              theme === "light" ? "#222" : "#fff"
            });
            flex-direction: column;
            gap: 8px;
            z-index: calc(var(--tl-z-index, 2147483647) + 1);
            }

            .tl-dropdown-panel.open {
            display: flex;
            }

            .tl-dropdown-panel .tl-search {
            width: 100%;
            padding: 8px;
            border-radius: 6px;
            ...
            .tl-dropdown-panel .tl-select-wrap select {
            width: 100%;
            padding: 8px;
            border-radius: 6px;
            border: 1px solid ${theme === "light" ? "#ddd" : "#444"};
            ...
            /* Make sure small screens show above button */
            @media (max-width: 480px) {
            .tl-dropdown-panel {
                right: 0;
                left: 16px;
    ...
    createContainer() {
      ...
      const cfgToggle = String(
        ((_b = (_a = this.options) == null ? void 0 : _a.config) == null
          ? void 0
          : _b.toggle) ?? "off"
      );
      const useIconMode = DomUtils.isToggleOn(cfgToggle);
      if (useIconMode) {
        this.container.classList.add("tl-floating-btn");
        const button = document.createElement("button");
        button.className = "tl-translate-btn";
        ...
        button.innerHTML = `
                <svg width="20" height="20" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
                  <path fill="currentColor" d="M12 3a9 9 0 100 18 9 9 0 000-18zm3.5 12h-2.01a8.93 8.93 0 01-1.2-2.5H15.5c.3.85.5 1.76.5 2.5zM9 8.5c.79 0 1.5.34 2.01.88C11.16 9.9 11 10.7 11 11.5c0 1.66 1.12 3.04 2.6 3.43-.24.6-.58 1.16-1.01 1.66A6.97 6.97 0 019 8.5z"></path>
                </svg>
            `;
        const panel = document.createElement("div");
        panel.className = "tl-dropdown-panel";
        panel.setAttribute("role", "menu");
        panel.setAttribute("data-no-translate", "true");
        panel.innerHTML = `
                <input type="text" class="tl-search" placeholder="Search language..." aria-label="Search languages" />
                <div class="tl-select-wrap"><select id="${SELECTORS.DROPDOWN}" data-no-translate="true" aria-label="Language select"></select></div>
            `;
        this.container.appendChild(button);
        this.container.appendChild(panel);
        document.body.appendChild(this.container);
        this.selectElement = this.container.querySelector(
          `#${SELECTORS.DROPDOWN}`
        );
        this.populateSelect(this.selectElement);
        ...
        if (this.container) {
          ...
          document.addEventListener(
            "click",
            (ev) => {
              const panelEl = container.querySelector(".tl-dropdown-panel");
              if (!panelEl) return;
              if (!container.contains(ev.target)) {
                panelEl.classList.remove("open");
                const btn = container.querySelector(".tl-translate-btn");
        ...
        const searchInput =
          (_c = this.container) == null
            ? void 0
            : _c.querySelector(".tl-search");
        if (searchInput instanceof HTMLInputElement && this.selectElement) {
          const selectEl = this.selectElement;
          searchInput.addEventListener("input", (event) => {
            const target = event.target;
            if (!target) return;
            const q = target.value.toLowerCase().trim();
            const { config, translatedDropdownLabels = {} } = this.options;
            if (!config || !config.languageLabels) return;
            const sourceLanguage = config.defaultLang;
            const available = new Set(
              [...(config.targetLanguages || []), sourceLanguage].filter(
                Boolean
              )
            );
            ...
            if (this.options.currentLang !== sourceLanguage) {
              ...
              for (const l of availableLanguages) {
                if (l !== sourceLanguage) allLanguages.push(l);
              }
            }
            const filtered = allLanguages.filter((l) => {
              const label = (
                translatedDropdownLabels[l] ||
                config.languageLabels[l] ||
                ""
              )
                .toString()
                .toLowerCase();
            ...
            for (const l of filtered) {
              ...
              option.value = l;
              option.textContent =
                translatedDropdownLabels[l] || config.languageLabels[l] || l;
              if (l === this.options.currentLang) option.selected = true;
        ...
        if (this.selectElement) {
          ...
          selectEl.addEventListener("change", () => {
            ...
            const onChange =
              (_a2 = this.options) == null ? void 0 : _a2.onLanguageChange;
