(function () {
  ...
  const EXTERNAL_SERVICES = {
    CIO_SITE_ID: "629416836761f52f409d",
  };
  const SELECTORS = {
  ...
  const initCIO = () => {
    ...
    (function () {
      ...
      t.async = true;
      t.id = "cio-tracker";
      t.setAttribute("data-site-id", EXTERNAL_SERVICES.CIO_SITE_ID);
      t.src = "https://assets.customer.io/assets/track.js";
      (_a = s.parentNode) == null ? void 0 : _a.insertBefore(t, s);
    })();
  };
  const trackCIOEvent = (eventName, data) => {
    if (typeof window === "undefined" || !window._cio) return;
    window._cio.push(["track", eventName, data]);
  };
  const BUILD_INFO = {
    version: "1.0.1",
  ...
  class TextTranslationEngine {
    ...
    async translatePage(targetLang) {
      ...
      trackCIOEvent("translation_page_start", {
      ...
      try {
        ...
        this.setTranslating(false);
        trackCIOEvent("translation_page_completed", {
          target_language: targetLang,
          nodes_count: fullTextNodes.length,
        });
      } catch (error) {
        skeletonMap.forEach((skeleton) => {
          SkeletonManager.remove(skeleton);
        ...
        this.setTranslating(false);
        trackCIOEvent("translation_page_failed", {
          target_language: targetLang,
    ...
    async translateBatch(payload) {
      ...
      try {
        ...
        trackCIOEvent("dynamic_translation_batch", {
          target_language: translateCfg.targetLanguage,
          node_count: allTexts.length,
        });
      } catch (error) {
    ...
    async switchLanguage(newLang) {
      ...
      try {
        ...
        trackCIOEvent("switch_language_failed", {
    ...
    async initialize() {
      ...
      try {
        initCIO();
        trackCIOEvent("text_translator_initialized", {
          domain: SCRIPT_CONFIG.domain,
          theme: SCRIPT_CONFIG.theme,
        });
      } catch (e) {
        console.error("❌ CIO initialization failed:", e);
      }
      this.startObserver();
      try {
        const apiConfig = this.configManager.getApiConfig();
